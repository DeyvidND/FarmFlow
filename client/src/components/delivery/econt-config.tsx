...
/**
 * Еконт-онлайн (auto mode) configuration — the live API integration body of the
 * courier card: credentials, sender profile, default package + COD fee payer,
 * advanced label options and the nomenclature sync. Extracted from the old
 * `econt-section` so it can live inside the «Доставка до адрес с куриер» card's
 * "Еконт-онлайн" sub-mode. The card owns the on/off and the manual/auto switch;
 * this component only renders once auto is selected.
 */
...
export function EcontAutoConfig({
  cfg,
  mut,
  toast,
}: {
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5">
        <p className={cn(subDescCls, 'mb-0 max-w-none')}>
          Свържи акаунта си в Еконт веднъж — после печаташ товарителници и смяташ цени направо оттук.
        </p>
      ...
      <div>
        ...
        <p className={subDescCls}>
          Въведи потребителя и паролата, които Еконт ти е дал за онлайн заявки. Намираш ги в профила
          си на econt.com или в договора — нямаш ли ги, обади се на Еконт.
        </p>
      ...
      {/* 3 — default package + COD fee payer */}
      <div>
        <h3 className={subHeadCls}>3. Пакет и наложен платеж</h3>
        <p className={subDescCls}>Стандартното тегло на пратките ти и кой поема таксата при наложен платеж.</p>
        <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
        ...
        <div className="mt-3.5 flex flex-wrap items-center justify-between gap-3 rounded-[10px] border border-ff-border bg-ff-surface-2 px-3.5 py-3">
          <div>
            <div className="text-[14px] font-bold text-ff-ink">Такса за наложен платеж</div>
            <div className="mt-px text-[12px] text-ff-muted">
              Включи ли клиентът наложен платеж, кой плаща таксата на Еконт за събирането.
            </div>
          </div>
